...
int mAh[5] = {0,0,0,0,0};
// Переменная используется в функции фиксации времени разряда аккумулятора
 unsigned long propertyForDischargeTime = 0;
// Если true, то аккумулятор не держит заряд
 bool isBatteryBad = false;
// счётчик числа нулевых разрядов, чтобы избежать бесмысленной работы над акб, который не заряжается
